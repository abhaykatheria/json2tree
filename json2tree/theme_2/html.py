...
def body_content_func_theme2(report_dict, indent, current_id_val=0, **kwargs):
    # This function will be called by base_create_html_report.
    # indent will be 0 as passed by the lambda.
    if isinstance(report_dict, dict):
        # dict_handler for Theme 2 creates its own surrounding <ul>
        return dict_handler(report_dict, indent, current_id_val=current_id_val, **kwargs)
    elif isinstance(report_dict, list):
        # list_handler for Theme 2 returns a string of <li> items (generated by item_prefix_theme2).
        # Reinstate wrapping these in a <ul> as per subtask.
        # Theme 2 typically uses plain <ul> for this level.
        list_html, next_id = list_handler(report_dict, indent, current_id_val=current_id_val, **kwargs)
        html_string = f"<ul>\n{list_html}</ul>\n"
        return html_string, next_id
    else:
        # Fallback for simple types as root.
        # Mimic the structure used for simple values within dicts for theme 2.
        # This will be placed inside the "ROOT" <li>.
        escaped_report_dict = str(report_dict).replace('<', '&lt;').replace('>', '&gt;')
        # Theme 2 uses 'tree_label' class for its text.
        return f"<ul><li><span class='tree_label'>{escaped_report_dict}</span></li></ul>", current_id_val
...
def create_html_report(report_dict, title="Report"):
    ...
    full_html = base_create_html_report(
        ...
        body_content_func=lambda rd, ind, cid: body_content_func_theme2(rd, ind, cid, **{}), # Pass empty dict for kwargs
